...
#Convergence Speed between Marshallian and Keynesian, day 2
...
Like the previous day, I am investigating why Keynesian reaches equilibrium somewhat faster.

###References

###Purpose
Identify why Keynesian reaches equilibrium faster than the rest


###Hypothesis
Something in the PID of the Keynesian pricing makes it faster than quantity adjustment in the Marshallian world (both use PID maximizers)
...
plot(ksales$maximizer_ratio)
#ksales<-head(ksales,n=1000)
plot(ksales$maximizer_ratio)
plot(ksales$maximizer_efficiency)

plot(ksales$maximizer_benefits, main="PID computed marginal benefits against theoretical ones",ylab="MB")
lines(ksales$offeredPrice/(2*sqrt(khr$inflow)),col="red")

head(khr)
plot(khr$inflow)
plot(ksales$outflow-ksales$inflow)
plot(ksales$stockouts)

plot(tail(khr$quota,n=3000))
plot(ksales$stockouts)
