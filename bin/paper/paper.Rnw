...
I base my model on Leijonhufvud's statement in "Keynes and the Keynesians" that the difference between Keynesian and Marshallian economics is  how agents react to disequilibrium. %CITE
To Leijonhufvud, Marshallian agents react to mismatches in demand by first changing prices and only later changing production. Keynesian agents instead react first by adjusting quantity and production and only later prices.
...
At the end of each day there is a small fixed probability (in this simulation $\frac {1}{20}$) of change labor input and therefore $y^s$.
The decision is simple marginal optimization: increase labor hired when $\text{MB}>\text{MC}$ and viceversa.
This is also done by trial and error and a PI controller where the error in this case is defined as $e_t = \frac{\text{MB}}{\text{MC}}-1$. 
...
Lab so a firm has only two degrees of freedom, $p$ and $L$, which are set by two independent PI controllers.
...
Marshallian firms perform as in the microeconomic scenario as they manipulate prices and so demand while keeping supply fixed.
Keynesian firms on the other hand manipulate labor directly and in a macroeconomics that changes both supply and demand at the same time, since demand is just total remuneration.
...
\label{sec:shock}
Because initial conditions matter, rather than studying the dynamics toward equilibrium ab ovo, I first let the model reach equilibrium then subject ti to a demand shock and see how the firms differ in adapting to it.
...
ggplot(data=surpluses) + geom_boxplot(aes(y=value,x=keynesian,fill=keynesian)) + 
  ...
  ggtitle("Surplus in Marshallian and Keynesian world ") + ylab("Surplus Difference from Immediate Equilibrium")
...
In other words, the Marshallian firm takes longer to get to the new equilibrium but proceeds over a more efficient path where demand and supply match for most of the time. Keynesian firms take fewer days but demand and supply never match while the new equilibrium is reached.
...
While overproduction serves a signalling purpose in this model as it pushes Keynesian firms to the new equilibrium quickly, it is an inherently wasteful and expensive signal that costs more to society than the slower alternative.
...
ggplot(data=full.metrics) + geom_boxplot(aes(y=value,x=keynesian,fill=keynesian)) + 
  ...
  theme_gray(30) + xlab("") + guides(fill=FALSE) + ggtitle("Comparison of equilibrium metrics with and without productivity shock ")
